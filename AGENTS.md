...
- **Frontend** (`pussalla-frontend/`): Vite + React 19, react-router-dom v7.
  ...
  - Pages: Login, Dashboard (role-aware), Earnings, DailyLogs, Tasks, Employees (super_admin can add users + set/reset passwords + assign super_admin role), CrossAssignments, Reports (monthly leaderboard + employee breakdown + all-employee work-breakdown grid with merged name/code cell, compact ✓ mode, **dedicated division filter defaulting to "All divisions"**; per-employee task×date grid; payslip PDF with count×amount; **on-screen `PayslipView`** (`src/components/PayslipView.jsx`) shown when "View grid" is clicked — max-w bounded card, bordered 2-col employee metadata, highlighted Total Incentive Payout summary, scrollable monthly task activity grid (sticky task/total cols) + zebra-striped Detailed Daily Breakdown table with Date/Task/Units+unit tag/Rate/Earnings), Audit, NotFound. The old "Breakdown" tab button + branch was removed (redundant with monthly "View grid").
